...
int main()
{
    ...
    //Initialize database:
    if (db_hdr_data_table_init()){
		printf("Error unable to initialize DB ... ");
		exit(1);
	}
...
void print_ethernet_header(const u_char *Buffer, int Size)
{
    ...
    memcpy(packet_header.mac_dst,tmp_buf,20);
    ...
    memcpy(packet_header.mac_src,tmp_buf,20);
    packet_header.eth_prot = eth->h_proto;
    packet_header.l2_size = Size;
}
...
void print_ip_header(const u_char * Buffer, int Size)
{
    ...
    memcpy(packet_header.ip_src,tmp_buf,20);
    
    memset(tmp_buf,0,20);
    sprintf(tmp_buf, "%s", inet_ntoa(dest.sin_addr));
    memcpy(packet_header.ip_dst,tmp_buf,20);
    packet_header.ip_prot = iph->protocol;
}
...
void print_tcp_packet(const u_char * Buffer, int Size)
{
    ...
    packet_header.port_src = ntohs(tcph->source);
    packet_header.port_dst = ntohs(tcph->dest);
...
void print_udp_packet(const u_char *Buffer , int Size)
{
    ...
    packet_header.port_src = ntohs(udph->source);
    packet_header.port_dst = ntohs(udph->dest);
